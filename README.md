...
虽称不上应有尽有，但基本的功能经已实现。你可以使用本博客系统进行文章的发布、后台管理等功能。
...
项目包含 master 及 canary 两个分支，请根据你的需要及喜好选择。
...
> 包含最新的、尚在开发中的特性，但不稳定，适合进阶用户及开发者。
...
3. Set your MySQL configuration and database name in the `dbconfig.php`. 在 `dbconfig.php` 配置好你的数据库信息。
4. You can login as admin with `username: admin` and `password: admin`. 你可以使用默认的管理员账户登录， `用户名：admin`，`密码：admin`。 
5. Enjoy yourself. 开始网上冲浪。
...
- [x] Add staticize generation 增加静态化生成器
