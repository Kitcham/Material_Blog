...
<head>
	<meta charset="UTF-8">
	<meta name=”description” content="Health Code"> 
	<meta name=”keyword” content="Health Code"> 
	<title>Health Code</title>
...
<body class="mdui-drawer-body-left mdui-appbar-with-toolbar  mdui-theme-primary-amber mdui-theme-accent-pink">
    ...
    <a href="index.php" class="mdui-typo-headline mdui-hidden-xs">Health Code</a>
    <div class="mdui-toolbar-spacer"></div>
    <div class="mdui-textfield mdui-textfield-expandable mdui-float-right"></div>
</div>
</header>
	...
	<div class="mdui-row">
	<div class="mdui-col-sm-4 mdui-col-md-4 mdui-col-offset-md-4">
						<div class='mdui-card mdui-hoverable'>
							...
							<?php
								...
								if(!$_SESSION){
									...
									$sql1 = "SELECT uid, uname, name, now_block, update_time FROM userinfo
											 WHERE uid='{$uid}'";
									...
									if($rows1){
										...
										$qruname = $rows1['uname'];
										$qrname = $rows1['name'];
										$qrblock_id = $rows1['now_block'];
										$update_time = $rows1['update_time'];
									}
									
									$sql2 = "SELECT block_id, block_risk FROM blockarea
											 WHERE block_id='{$qrblock_id}'";
									 $smt2 = $pdo->prepare($sql2);
									 $smt2->execute();
									$rows2= $smt2->fetch();
									 if($rows2){
										 $qrrisk = $rows2['block_risk'];
									}
									
									$data = $qruid . "|" . $qrname . "|" . $qruname . "|" . $qrblock_id . "|" . $qrrisk . "|" . $update_time;
									 //二维码结束
									echo "
									<img class='mdui-card-header-avatar' src='$avatar'/>
									 <div class='mdui-card-header-title'>$qruname</div>
									 <div class='mdui-card-header-subtitle'>居民</div>
								</div>
								<div class='mdui-card-media'>";
								if($qrrisk == "0"){
									echo "<img src='http://qr.topscan.com/api.php?&bg=ffffff&fg=90B44B&text=$data'/>";
								}
								else if($qrrisk == "1"){
									echo "<img src='http://qr.topscan.com/api.php?&bg=ffffff&fg=E98B2A&text=$data'/>";
								}
								else if($qrrisk == "2"){
									echo "<img src='http://qr.topscan.com/api.php?&bg=ffffff&fg=CB4042&text=$data'/>";
								}
								else{
									echo "无信息！！！！";
								}
								...
								<div class='mdui-card-primary'>
									 ...
									 <div class='mdui-card-primary-title'>$qrname</div>
									 <div class='mdui-card-primary-subtitle'>$update_time</div>
								</div>
								<div class='mdui-card-content'>$data</div>";
								}
						?>
						</div>
				</br></br></div>
