...
//插入用户信息sql
$sql = "INSERT INTO user(uname, password, is_admin, email) VALUES('{$username}','{$pwd}','0','{$email}')";
...
if($pdo->exec($sql)){
	...
	$rows= $smt->fetch();
	$uid=$rows['uid'];
	$sql2 = "INSERT INTO userinfo(uid, uname, name, phone, email, now_block, infection, avatar) 
			 VALUES('{$uid}','{$username}','0','0','{$email}','0','0','{$target_name}')";
