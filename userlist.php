...
<body class="mdui-drawer-body-left mdui-appbar-with-toolbar  mdui-theme-primary-amber mdui-theme-accent-pink">
  ...
  <div class="mdui-toolbar mdui-color-theme">
	...
	<div class="mdui-dialog" id="addDialog">
		...
		<div class="right">
			<!--查看用户-->
			<div style="display:block mdui-table-fluid" class="m">
				<?php
				session_start();
				if(!$_SESSION['username']){
					echo "<script>location='index.php'</script>";
				...
				<table width=900  border=1 style="border-collapse:collapse" cellspacing="0" cellpadding="6px" class="table-select-uer mdui-table mdui-table-hoverable">
					...
					foreach($rows as $r){
						...
						echo "<td>
						<button class=\"mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple change\" mdui-dialog=\"{target: '#delConfirmDialog'}\">移除账户</button>";
						echo "</tr>";
						echo "<div class=\"mdui-dialog\" id=\"delConfirmDialog\">
  							<div class=\"mdui-dialog-title\">确定移除该用户？</div>
  							<div class=\"mdui-dialog-content\">此操作不可回退，无法恢复！</div>
  							<div class=\"mdui-dialog-actions\">
    							<button class=\"mdui-btn mdui-ripple\" num='#' mdui-dialog-close>取消</button>
    							<button class=\"mdui-btn mdui-ripple del\" num='{$r['id']}' mdui-dialog-confirm>确定移除</button>
  							</div>
						</div>";
					}
					?>
				...
				<button class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple change" mdui-dialog="{target: '#fixDialog'}">修改用户密码</button>
			</div>
		
	<!--修改密码-->
	<div class="mdui-dialog changeform" id="fixDialog">
    	<div id="example4-tab1" class="mdui-p-a-2">
			<div class="mdui-typo-display-title-opacity">修改用户密码&nbsp&nbsp&nbsp<mark>风险权限，谨慎操作！！!</mark></div>
			<!--<a href="#example4-tab1" class="mdui-ripple">添加用户</a> -->
...
<					</label>
    		...
    		<div class="mdui-col">
      			<button class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple">确认修改</button>
